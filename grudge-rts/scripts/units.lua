--  Grudge RTS - Unit Definitions
--  All Grudge units use NumDirections = 1 (side-view sprites)

DebugPrint("Loading Grudge units...\n")
...
-- Frames: idle 0-5, walk 6-13, attack 14-20, death 21-24
local KnightStill = {"label start", "frame 0", "wait 6", "frame 1", "wait 6", "frame 2", "wait 6", "frame 3", "wait 6", "frame 4", "wait 6", "frame 5", "wait 6", "goto start"}
...
DefineUnitType("unit-grudge-knight", {
  ...
  --Icon = "icon-knight",
  NumDirections = 1,
  Offset = {0, -16},  -- Shift sprite up so feet align with tile
  Speed = 10,
  HitPoints = 60,
  DrawLevel = 40,
...
DebugPrint("Grudge units loaded.\n")
